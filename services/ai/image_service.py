...
"""
from vertexai.generative_models import Part, HarmCategory, HarmBlockThreshold
    ...
    def _initialize_model(self):
        ...
        try:
            if self.config.image_model_name:
                ...
                logger.info(f"Image generation model '{self.config.image_model_name}' loaded for AIImageService.")
        except Exception as e:
            logger.error(f"AIImageService model initialization failed: {e}", exc_info=True)
...
請將你的分析整理成一段流暢的描述。"""
        response = self.core_service.text_vision_model.generate_content([image_part, prompt])
    ...
    def translate_prompt_for_drawing(self, prompt_in_chinese: str) -> str:
        ...
        try:
            ...
            response = self.core_service.text_vision_model.generate_content(translation_prompt)
            return self.core_service.clean_text(response.text)
        except Exception as e:
