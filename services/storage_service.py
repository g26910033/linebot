...
class StorageService:
    ...
    def __init__(self, config):
        ...
        # Cloudinary 初始化
        try:
            if all([config.cloudinary_cloud_name,
                ...
                logger.warning(
                    "Cloudinary credentials not fully provided. Image upload will be disabled.")
        except Exception as e:
            self.cloudinary_enabled = False
            logger.error(
                f"Cloudinary configuration failed: {e}",
                exc_info=True)
    ...
    def upload_image(self, image_bytes: bytes) -> Tuple[Optional[str], str]:
        ...
        try:
            ...
            logger.error(
                f"Failed to upload image to Cloudinary: {e}",
                exc_info=True)
            return None, str(e)
    ...
    def get_chat_history(self, user_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(
                f"Failed to get chat history for {user_id}: {e}",
                exc_info=True)
            return []
    ...
    def save_chat_history(self, user_id: str,
        ...
        try:
            ...
            self.redis_client.set(
                key, json.dumps(history), ex=self.chat_history_ttl)
        except redis.exceptions.RedisError as e:
            logger.error(
                f"Failed to save chat history for {user_id}: {e}",
                exc_info=True)
    ...
    def clear_chat_history(self, user_id: str) -> None:
        ...
        try:
            ...
            logger.error(
                f"Failed to clear chat history for {user_id}: {e}",
                exc_info=True)
    ...
    def set_user_last_image_id(self, user_id: str, message_id: str) -> None:
        ...
        try:
            ...
            logger.error(
                f"Failed to set last image ID for {user_id}: {e}",
                exc_info=True)
    ...
    def get_user_last_image_id(self, user_id: str) -> Optional[str]:
        ...
        try:
            ...
            logger.error(
                f"Failed to get last image ID for {user_id}: {e}",
                exc_info=True)
            return None
    ...
    def set_user_state(self, user_id: str, state: str) -> None:
        ...
        try:
            ...
            logger.error(
                f"Failed to set user state for {user_id}: {e}",
                exc_info=True)
    ...
    def get_user_state(self, user_id: str) -> Optional[str]:
        ...
        try:
            ...
            logger.error(
                f"Failed to get user state for {user_id}: {e}",
                exc_info=True)
            return None
    ...
    def set_nearby_query(self, user_id: str, query: str) -> None:
        ...
        try:
            ...
            logger.error(
                f"Failed to set nearby query for {user_id}: {e}",
                exc_info=True)
    ...
    def get_nearby_query(self, user_id: str) -> Optional[str]:
        ...
        try:
            ...
            logger.error(
                f"Failed to get nearby query for {user_id}: {e}",
                exc_info=True)
            return None

    def set_user_last_location(
            self,
            user_id: str,
            latitude: float,
        ...
        try:
            ...
            logger.error(
                f"Failed to set last location for {user_id}: {e}",
                exc_info=True)

    def get_user_last_location(
            self, user_id: str) -> Optional[Dict[str, float]]:
        if not self.redis_client:
            return None
        key = self._get_key(user_id, "last_location")
        try:
            ...
            logger.error(
                f"Failed to get last location for {user_id}: {e}",
                exc_info=True)
            return None
    ...
    def add_todo_item(self, user_id: str, item: str) -> bool:
        ...
        try:
            ...
            logger.error(
                f"Failed to add todo item for {user_id}: {e}",
                exc_info=True)
            return False
    ...
    def get_todo_list(self, user_id: str) -> List[str]:
        ...
        try:
            ...
            logger.error(
                f"Failed to get todo list for {user_id}: {e}",
                exc_info=True)
            return []
    ...
    def remove_todo_item(self, user_id: str, index: int) -> Optional[str]:
        ...
        try:
            ...
            logger.error(
                f"Failed to remove todo item for {user_id}: {e}",
                exc_info=True)
            return None
