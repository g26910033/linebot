...
class UtilityService:
    ...
    def __init__(self):
        ...
        # 建立所有單位的列表，用於正則表達式
        all_units = list(self.length_units.keys()) + \
    ...
    def parse_and_convert(self, text: str, parsing_service=None) -> str | None:
        ...
        # 優先嘗試貨幣換算 (透過 AI 服務)
        if parsing_service:
            currency_query = parsing_service.parse_currency_conversion_query(
                text)
            if (currency_query and currency_query.get("value") and
                    currency_query.get("from_currency") and
                    currency_query.get("to_currency")):
                try:
                    ...
                    logger.error(
                        f"Error during AI-assisted currency conversion: {e}")
                    return "抱歉，匯率換算時發生錯誤。"
