...
"""
import threading
from linebot.v3.messaging import MessagingApi, TextMessage, TemplateMessage
...
class AIIntentHandler:
    ...
    def __init__(self, parsing_service: AIParsingService, text_service: AITextService, weather_service: WeatherService, news_service: NewsService, stock_service: StockService, calendar_service: CalendarService, line_bot_api: MessagingApi):
    ...
    def handle(self, user_id: str, user_message: str):
        ...
        # 天氣查詢
        weather_query = self.parsing_service.parse_weather_query_from_text(user_message)
        if weather_query and weather_query.get("city"):
            self._handle_weather(user_id, weather_query)
            return True

        # 股票查詢
        symbol = self.parsing_service.parse_stock_symbol_from_text(user_message)
        if symbol:
        ...
        # 日曆查詢
        if any(keyword in user_message.lower() for keyword in ["提醒我", "新增日曆", "新增行程", "的日曆"]):
    ...
    def _handle_stock(self, user_id, symbol):
        def task():
            ...
            self.line_bot_api.push_message(to=user_id, messages=[TextMessage(text=result)])
        threading.Thread(target=task).start()
    ...
    def _handle_calendar(self, user_id, user_message):
        def task():
            event_data = self.parsing_service.parse_event_from_text(user_message)
            ...
            self.line_bot_api.push_message(to=user_id, messages=[TextMessage(text=reply_text)])
        threading.Thread(target=task).start()

    def _handle_translation(self, user_id, user_message):
        def task():
            translated_text = self.text_service.translate_text(user_message)
            self.line_bot_api.push_message(to=user_id, messages=[TextMessage(text=translated_text)])
        threading.Thread(target=task).start()
