...
class TodoCommandHandler:
    ...
    def handle_add(self, user_id: str, reply_token: str, item: str):
        """處理新增待辦事項。"""
        if not item:
            self.line_bot_api.reply_message(reply_token, messages=[TextMessage(text="請告訴我要新增什麼待辦事項喔！\n格式：`新增待辦 買牛奶`")])
            return
        if self.storage_service.add_todo_item(user_id, item):
            self.line_bot_api.reply_message(reply_token, messages=[TextMessage(text=f"好的，已將「{item}」加入您的待辦清單！")])
        else:
            self.line_bot_api.reply_message(reply_token, messages=[TextMessage(text="抱歉，新增待辦事項時發生錯誤。")])

    def handle_list(self, user_id: str, reply_token: str):
        """處理列出待辦事項。"""
        todo_list = self.storage_service.get_todo_list(user_id)
        if not todo_list:
            self.line_bot_api.reply_message(reply_token, messages=[TextMessage(text="您的待辦清單是空的！")])
        else:
            flex_message_dict = self._create_todo_list_flex_message(todo_list)
            # _reply_flex_message 邏輯需要被提取或重構
            # 暫時簡化處理
            self.line_bot_api.reply_message(reply_token, messages=[TextMessage(text="這是您的待辦清單：\n" + "\n".join(f"{i+1}. {item}" for i, item in enumerate(todo_list)))])
    ...
    def handle_complete(self, user_id: str, reply_token: str, text: str):
        ...
        if item_index < 0:
            self.line_bot_api.reply_message(reply_token, messages=[TextMessage(text="請告訴我要完成哪一項喔！\n格式：`完成待辦 1`")])
            return
            
        removed_item = self.storage_service.remove_todo_item(user_id, item_index)
        if removed_item is not None:
            self.line_bot_api.reply_message(reply_token, messages=[TextMessage(text=f"太棒了！已完成項目：「{removed_item}」")])
        else:
            self.line_bot_api.reply_message(reply_token, messages=[TextMessage(text="找不到您指定的待辦事項，請檢查編號是否正確。")])
    ...
    def _create_todo_list_flex_message(self, todo_list: list) -> dict:
        ...
        for i, item in enumerate(todo_list[:10]):
            body_contents.append({
                ...
                "contents": [
                    {"type": "text", "text": f"{i+1}. {item}", "wrap": True, "flex": 4},
                    {
        ...
        return {
            "type": "bubble",
            "header": {"type": "box", "layout": "vertical", "contents": [{"type": "text", "text": "📝 您的待辦清單", "weight": "bold", "size": "xl", "color": "#1DB446"}]},
